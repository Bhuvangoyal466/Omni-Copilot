...
from db.models import SessionLocal, ToolConnection
    ...
    try:
        with SessionLocal() as session:
            return (
                ...
                .filter(
                    ToolConnection.user_id == user_key,
                    ToolConnection.tool_name == "provider:google",
                    ToolConnection.connected.is_(True),
                )
                .one_or_none()
            )
    except Exception:
...
def _persist_google_token(user_id: str, token_payload: dict[str, Any]) -> None:
    ...
    try:
        with SessionLocal() as session:
            ...
            row.encrypted_token = _encrypt_token_blob(token_payload)
            session.commit()
    except Exception:
        return
