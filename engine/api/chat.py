...
from agents.router import run_orchestration
    ...
    async def event_generator():
        try:
            user_id = (
                user.get("sub")
                if user
                else request.headers.get("x-omni-user-id", "anonymous")
            )
            memory_hits = await memory_store.search_memory(
                payload.message, limit=3, user_id=user_id
            )
            ...
            async for token_event in stream_text_tokens(
                state["answer"], delay=token_delay
            ):
                yield token_event
