...
from db.models import ToolConnection, ensure_user, get_tool_connection, upsert_tool_connection
...
INTEGRATION_TOOLS: list[IntegrationTool] = [
    ...
    IntegrationTool(id="gsheets", label="Google Sheets", provider="google", persona="Sheet automation"),
...
async def list_integrations(
    ...
    for tool in INTEGRATION_TOOLS:
        provider_row = get_tool_connection(user_key, _provider_key(tool.provider))
...
async def disconnect_tool(
    ...
    provider_row = upsert_tool_connection(user_id=user_key, tool_name=provider_key, connected=False, encrypted_token=None)
    for related_tool in INTEGRATION_TOOLS:
        if related_tool.provider == tool.provider:
            upsert_tool_connection(user_id=user_key, tool_name=related_tool.id, connected=False, encrypted_token=None)
...
async def oauth_callback(
    ...
    provider_row = upsert_tool_connection(
        user_id=user_id,
    ...
    for related_tool in INTEGRATION_TOOLS:
        if related_tool.provider == provider_name:
            upsert_tool_connection(user_id=user_id, tool_name=related_tool.id, connected=True, encrypted_token=None)
